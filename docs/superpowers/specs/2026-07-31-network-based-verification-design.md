...
对多数方案（UTXO/EVM/ECDSA 及做地址推导的 Ed25519 链），签名必须密码学地对应到该行声明的地址/公钥，声明错误的 network 只会导致验证失败。

**残余风险（终审确认，遗留自旧实现并因解耦而扩大作用面）**：Ed25519 路径中 NEAR/HBAR/SC/IOTA 等链只验证"签名与自带公钥自洽"，不做地址绑定（`verify.go` 中这些 case 直接 return nil）。coin 列与 network 列解耦后，报告生成方理论上可对任意 coin 声明 `network=HBAR` 等值，用一对自洽的 ed25519 密钥使该行通过验签，而余额仍按 coin 列汇总。改造前 coin 名强制绑定验签方案，此路径仅限这些链自身；改造后对全部资产开放。德勤流程（不看 coin 列）行为不变。需后续任务硬化：coin↔链族一致性校验，或为这些链补地址推导。
